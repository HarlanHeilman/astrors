...
use crate::io::{
    hdus::bintable::buffer::Buffer, header::card::Card, utils::pad_buffer_to_fits_block,
    utils::pad_read_buffer_to_fits_block, Header,
};
            ...
            .map(|(start, end)| {
                ...
                (0..nbuffer_rows).into_iter().for_each(|i| {
                    ...
                    columns.iter().enumerate().for_each(|(j, column)| {
                        ...
                        let col_bytes =
                            &row[column.start_address..column.start_address + column.type_bytes];
        ...
        .map(|series| {
            ...
            let tform = match series.dtype() {
                ...
                DataType::String => {
                    ...
                    let mut max_length = data
                        .iter()
                        .map(|item| item.unwrap_or("").len())
                        .max()
                        .unwrap();
            ...
            let column: Column = Column::new(ttype.to_string(), tform, None, None, start_address);
...
pub fn create_table_on_header(header: &mut Header, columns: &Vec<Column>, nrows: i64) {
    ...
    header.add_card_on_index(
        &Card::new(
            "XTENSION".to_string(),
            "BINTABLE".to_string(),
            Some("Binary table".to_string()),
        ),
        0,
    );
    header.add_card_after(
        &Card::new(
            "BITPIX".to_string(),
            8.to_string(),
            Some("Table BITPIX".to_string()),
        ),
        "XTENSION",
    );
    header.add_card_after(
    ...
    header.add_card_after(
        &Card::new(
            "NAXIS1".to_string(),
            num_bytes.to_string(),
            Some("Number of bytes in row".to_string()),
        ),
        "NAXIS",
    );
    header.add_card_after(
    ...
    header.add_card_after(
        &Card::new(
            "PCOUNT".to_string(),
            0.to_string(),
            Some("Parameter count".to_string()),
        ),
        "NAXIS2",
    );
    header.add_card_after(
    ...
    header.add_card_after(
        &Card::new(
            "TFIELDS".to_string(),
            tfields.to_string(),
            Some("Number of fields per row".to_string()),
        ),
        "GCOUNT",
    );
    for (i, column) in columns.iter().enumerate() {
        header.add_card(&Card::new(
            format!("TTYPE{}", i + 1),
            column.ttype.clone(),
            Some("Name of field".to_string()),
        ));
        header.add_card(&Card::new(
            format!("TFORM{}", i + 1),
            column.tform.clone(),
            Some("Format of field".to_string()),
        ));
        if let Some(tunit) = &column.tunit {
            header.add_card(&Card::new(
                format!("TUNIT{}", i + 1),
                tunit.clone(),
                Some("Unit of field".to_string()),
            ));
        }
        if let Some(tdisp) = &column.tdisp {
            header.add_card(&Card::new(
                format!("TDISP{}", i + 1),
                tdisp.clone(),
                Some("Display format of field".to_string()),
            ));
        }
    }
}
            ...
            .map(|(start, end)| {
                ...
                columns.iter().for_each(|column| {
                    ...
                    match series.dtype() {
                        DataType::Boolean => {
                            series
                                ...
                                .for_each(|(j, item)| {
                                    ...
                                    local_buffer[col_start_add..col_start_add + column.type_bytes]
                        ...
                        DataType::List(dtype) => match dtype.to_physical() {
                            DataType::Boolean => {
                                series
                                    .list()
                                    ...
                                    .for_each(|(j, item)| {
                                        ...
                                        array.iter().enumerate().for_each(|(k, it)| {
                                            if let AnyValue::Boolean(item) = it {
                                                ...
                                                local_buffer
                                                    [col_start_add + k..col_start_add + k + 1]
                                                    .copy_from_slice(&[bytes]);
                                            }
                                        });
                                    });
                                    ...
                                    .for_each(|(j, item)| {
                                        ...
                                        array.iter().enumerate().for_each(|(k, it)| {
                                            if let AnyValue::Int16(item) = it {
                                                ...
                                                local_buffer[col_start_add + k * 2
                                                    ..col_start_add + k * 2 + 2]
                                                    .copy_from_slice(&bytes.to_be_bytes());
                                            }
                                        });
                                    ...
                                    .for_each(|(j, item)| {
                                        ...
                                        array.iter().enumerate().for_each(|(k, it)| {
                                            if let AnyValue::Int32(item) = it {
                                                ...
                                                local_buffer[col_start_add + k * 4
                                                    ..col_start_add + k * 4 + 4]
                                                    .copy_from_slice(&bytes.to_be_bytes());
                                            }
                                        });
                                    ...
                                    .for_each(|(j, item)| {
                                        ...
                                        array.iter().enumerate().for_each(|(k, it)| {
                                            if let AnyValue::Int64(item) = it {
                                                ...
                                                local_buffer[col_start_add + k * 8
                                                    ..col_start_add + k * 8 + 8]
                                                    .copy_from_slice(&bytes.to_be_bytes());
                                            }
                                        });
                        ...
                        DataType::String => {
                            series
                                ...
                                .for_each(|(j, item)| {
                                    ...
                                    let col_start_add = row_start_add + column.start_address;
                                    local_buffer[col_start_add..col_start_add + column.type_bytes]
                                        .copy_from_slice(&string.into_bytes());
                                });
                        }
                        _ => {
                            series
